...
const oldModalStart = content.indexOf('{/* DEPOSIT MODAL POPUP */}');
...
if (oldModalStart !== -1 && oldModalEnd !== -1) {
  ...
  content = content.substring(0, oldModalStart) + newModal + '\n    </div>\n  );\n};\n';
  fs.writeFileSync('src/components/BillingModal.tsx', content);
  console.log('Modal replaced');
} else {
  console.log('Failed to find old modal', oldModalStart, oldModalEnd);
}
